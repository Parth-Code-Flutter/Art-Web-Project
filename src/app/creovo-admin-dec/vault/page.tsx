...
export default function AdminDashboard() {
    ...
    const [activeTab, setActiveTab] = useState<'products' | 'categories' | 'applications' | 'shipping'>('products');
    ...
    async function fetchData() {
        ...
        try {
            if (activeTab === 'shipping') {
                setLoading(false); // Handled by component mostly, but just in case
                return;
            }
            ...
            `}>
                ...
                <nav className="flex-1 px-4 space-y-2 mt-4">
                    ...
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-300 group ${activeTab === 'shipping' ? 'bg-white/10 text-white shadow-[0_0_20px_rgba(255,255,255,0.05)]' : 'text-zinc-500 hover:text-zinc-200 hover:bg-white/5'}`}
                    >
                        <div className={`p-2 rounded-lg transition-colors ${activeTab === 'shipping' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-900 group-hover:bg-zinc-800'}`}>
                            <Globe size={18} />
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4">
                        <div className="relative group w-full md:w-auto">
                            ...
                            <input
                                ...
                                className={`bg-zinc-900/50 border border-white/5 rounded-2xl pl-12 pr-4 py-3 w-full md:w-[300px] focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/10 transition-all placeholder:text-zinc-600 text-sm ${activeTab === 'shipping' ? 'opacity-0 pointer-events-none' : ''}`}
                            />
                        </div>

                        {activeTab !== 'applications' && activeTab !== 'shipping' && (
                            <button
